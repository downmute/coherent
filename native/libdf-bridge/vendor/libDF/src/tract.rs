...
impl DfParams {
    ...
    pub fn from_dir(dir: &Path) -> Result<Self> {
        ...
        File::open(dir.join("enc.onnx"))
            .context("Could not open enc.onnx from model directory.")?
            .read_to_end(&mut enc)
            .context("Could not read enc.onnx from model directory.")?;
        File::open(dir.join("erb_dec.onnx"))
            .context("Could not open erb_dec.onnx from model directory.")?
            .read_to_end(&mut erb_dec)
            .context("Could not read erb_dec.onnx from model directory.")?;
        File::open(dir.join("df_dec.onnx"))
            .context("Could not open df_dec.onnx from model directory.")?
            .read_to_end(&mut df_dec)
            .context("Could not read df_dec.onnx from model directory.")?;

        let config = Ini::load_from_file(dir.join("config.ini"))
            .context("Could not load config.ini from model directory.")?;
